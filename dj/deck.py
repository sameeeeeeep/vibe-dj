...
class Deck:
    def __init__(self, name: str):
        ...
        self.rate = 1.0         # playback speed (1.0 == original tempo)
    ...
    def effective_bpm(self) -> float:
        if not self.analysis:
            return 0.0
        return self.analysis.bpm * self.rate
    ...
    def remaining_sec(self) -> float:
        with self._lock:
            ...
            if n == 0 or self.rate <= 0:
                return 0.0
            return max(0.0, (n - self.pos) / (SAMPLE_RATE * self.rate))
    ...
    def seconds_to_next_beat(self) -> float:
        ...
        return src_to_next / self.rate                  # compress by varispeed
    ...
    def read(self, n: int) -> np.ndarray:
        ...
        with self._lock:
            ...
            if not self.playing or length == 0:
                return out
            positions = self.pos + np.arange(n) * self.rate
            self.pos = self.pos + n * self.rate
