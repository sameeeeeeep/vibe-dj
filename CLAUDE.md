...
## ✅ DONE (recent)
- **Listen-link QR popover + per-track "dig 3"** — two dashboard UX additions.
  (1) The topbar LISTEN pill is now a button that opens a popover with a **scannable QR** of the LAN
  listen URL (guests point a phone camera at it) plus the URL as big copyable text, a COPY LINK
  button, and an OPEN ↗ link; caption warns "restart with --lan" when not LAN-bound (`broadcast.lan`
  in the snapshot). QR generated by a new dependency-free encoder `dj/qrcode.py` (numpy only:
  GF(256)/Reed-Solomon, byte mode, EC level L, versions 1-4 single-block — covers any
  `http://<ip>:<port>/listen`; picks smallest version + lowest-penalty mask). Served at
  `GET /listen.qr.svg` (`Dashboard.listen_qr_svg()`/`listen_url()`). **Verified**: the encoder
  round-trips through OpenCV's `QRCodeDetector` for 6 URLs (v1-v3), and the live-served SVG decodes
  back to the exact LAN URL. (2) Every **queue row AND crate row** carries a ★ "dig 3 like this"
  button (`digFrom(tid)`), beside the global header DIG 3. `scout.dig_now(count, seed=)` / `_dig(...,
  seed=)` now take an optional seed track; with a seed the YouTube-Mix/search is built from *that*
  track and the tempo gate uses *its* BPM (live deck's heard BPM only when seeding off the live
  song). Dashboard `dig_similar` cmd carries the `tid`; `_dig_similar(count, tid)` resolves it via
  `controller._resolve_tid`. **Verified live**: a dig posted for a non-live crate track logged
  `digging 3 — search 'warmup_100bpm' (~99 BPM)`, seeding off the chosen track, not the 123-BPM live
  one. NOTE: not yet committed (earlier commit covered the self-clean/LAN/jukebox batch only).
...
dj/
  ...
  qrcode.py        dependency-free QR encoder (numpy only: GF(256)/Reed-Solomon, byte mode, EC
                   level L, versions 1-4 single-block). matrix()/svg(). Used by the dashboard's
                   GET /listen.qr.svg so guests can scan a phone at the listen link. Verified by
                   round-tripping through OpenCV's QRCodeDetector.
